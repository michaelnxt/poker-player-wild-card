...
game_state_many_players={
    ...
    "players": [                                    // An array of the players. The order stays the same during the
        ...
        {
            "id": 3,                                // Your own player looks similar, with one extension.
            "name": "Bob",
            "status": "active",
            "version": "Default random player",
            "stack": 1590,
  ...
  game_state_good_card_on_the_table={
    "tournament_id":"550d1d68cd7bd10003000003",     // Id of the current tournament
    "game_id":"550da1cb2d909006e90004b1",           // Id of the current sit'n'go game. You can use this to link a
                                                    // sequence of game states together for logging purposes, or to
                                                    // make sure that the same strategy is played for an entire game
...
test('many players no bet', () => {
  ...
  Player.betRequest(game_state_many_players,  bet);
  expect(result === 0);
});
test('good card on the table', () => {
  let result;
  function bet (amount){
    result = amount;
  }
  Player.betRequest(game_state_good_card_on_the_table,  bet);
  expect(Number.isInteger(result)=== true);
  expect(result>0);
});
